...
function Sidebar(props) {
  ...
  const createLinks = (routes) => {
    ...
    return routes.map((prop, key) => {
      const {roles = []} = prop;
      if ((prop.hide || !roles.includes(_.get(currentUser, 'customClaims.role', 'user')))) {
        return null;
      }
      if (prop.redirect) {
        return null;
          ...
          >
            ...
            <Collapse in={state[prop.state]} unmountOnExit>
              <List className={classes.list + " " + classes.collapseList}>
                {currentUser && createLinks(prop.views)}
              </List>
            </Collapse>
          </ListItem>
        );
  ...
  var user = (
    <div className={userWrapperClass}>
      <div className={photo}>
        {authUser && authUser.email && <img src={`https://www.gravatar.com/avatar/${crypto.createHash('sha1').update(authUser.email).digest('hex')}`} className={classes.avatarImg} alt="..." />}
      </div>
      <List className={classes.list}>
        <ListItem className={classes.item + " " + classes.userItem}>
          ...
          >
            {authUser && authUser.email && <ListItemText
